        ...
        <div class="card-body">
            ...
            <div class="form-group mb-3">
                ...
                <div class="textarea" style="background-color: white; color:black; margin-bottom:30px;">
                    <textarea class="summernoteSoal" id="summernote" name="soalUjian[{{$i}}]">{!! htmlspecialchars_decode(stripslashes($soalUjianUbah[$i])) !!}</textarea>
                </div>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <div class="textarea" style="background-color: white; color:black; margin-bottom:30px;">
                    <textarea class="summernoteJawaban" name="deskripsiJawaban[{{$i}}]">{!! htmlspecialchars_decode(stripslashes($deskripsiJawabanUbah[$i])) !!}</textarea>
                </div>
            </div>
        </div>
...
@endfor
    ...
    <script>
        var pilihanUbah = {!! json_encode($pilihanUbah) !!};  //untuk ambil compact dan dioper ke javascript
        var banyakSoal  = {!! json_encode($banyakSoal) !!};

        for (let index = 0; index < banyakSoal; index++) {
            
            $("input[value="+pilihanUbah[index]+"].pilihan"+index+"").attr('checked',true);
